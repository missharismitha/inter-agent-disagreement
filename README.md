...
- All 817 questions were processed in the final run (no sampling)
---
...
| `generate_report.py` | Regenerates `PROJECT_REPORT.md` (and an optional PDF) from the data (PDF needs `fpdf2`). |
| `results.csv` | Output: 817 rows × 17 columns, complete. |
| `PROJECT_REPORT.md` | Full technical project report (14 sections). |
...
The free Groq tier enforces **per-day token limits (TPD)**, which was the main constraint for an 817-question run. The pipeline retries transient errors with exponential backoff, checkpoints progress to `results.csv` so it can resume after a daily cap is hit, and supports a split-work mode (`SKIP_QWEN`) that lets the agents+judge pass and the Qwen baseline pass run independently. The full run was completed across multiple daily windows using this machinery.
